...
The [serial](/reference/serial) supports [serial communication](https://en.wikipedia.org/wiki/Serial_port) between the BBC Ragga and another computer. Basically, this allows you to send data from the Ragga to your own computer. This is very useful for debugging purposes: you can add `write line` statements in your code and see them display on your computer as the program executes.

The code below shows a simple script that sends a line when the BBC Ragga starts and another line each time the button ``A`` is pressed.
...
## How to read the Ragga's serial output from your computer
...
If you are running a Windows version earlier than 10, you must [install a device driver](https://os.mbed.com/docs/latest/tutorials/windows-serial-driver.html) (for the computer to recognize the serial interface of the Ragga).
...
Once both the driver and the terminal emulator are installed, plug in the Ragga and wait until the device is fully setup. Then, open TeraTerm.
...
Please note that Windows will assign you a different COM port if you plug in another Ragga. If you're juggling between micro:bits, you'll have to change the COM port every time.
...
* Plug in the Ragga.
* Open a terminal.
* Find which device node the Ragga was assigned to with the command `ls /dev/ttyACM*`.
...
* `screen /dev/cu.usbmodem1422 115200` will open up the Ragga's serial output. To exit, hit `Ctrl-A` `Ctrl-D`.
