...
* Code the Ragga and perform a series of tests dropping the Ragga in the carriers from a height.
...
This project will use the Ragga to test the force of collision.

* From the ``||input:Input||`` Toolbox drawer, drag an ``||input:on shake||`` block to the workspace
* Use the drop-down menu to select 8g. This block will detect when a force 8g or greater is exerted on the Ragga.
...
* Draw an X or other symbol to indicate that the Ragga has experienced 8g of force.
...
After coding the Ragga and constructing the egg drop carriers, take turns testing dropping the Ragga in the carriers from a height. Do the Ragga lights turn on? If so, that means the force exerted on the Ragga was at least 8g – a good indication that the egg most likely will break on impact. Continue refining the egg drop carriers until no Ragga lights turn on when dropped. Then test with an egg!
...
* Were the Ragga test results a good indication of whether the egg would break or not?
